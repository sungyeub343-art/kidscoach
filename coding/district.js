...
if (content) {
  document.title = `${pageAreaName} 코딩 과외 | 키즈상상코칭`;
  ...
  content.innerHTML = `
    <section class="district-hero container reveal visible"><div><a class="breadcrumb" href="index.html">코딩 과외</a><p class="eyebrow">${regionName} ${districtName} 맞춤 코딩</p><h1>${districtName}에서<br /><span>아이의 첫 코딩 프로젝트</span>를 시작하세요</h1><p class="hero-text">키즈상상코칭은 ${districtName} 가정의 일정과 아이의 관심사를 바탕으로 유아·초등 코딩 과외를 맞춤 연결합니다.</p><div class="hero-cta"><a class="btn" href="../#contact">${districtName} 코딩 상담</a><a class="btn btn-ghost" href="index.html">다른 지역 찾기</a></div></div><aside class="district-fact"><span class="fact-label">${regionName} · ${districtName}</span><strong>만들고<br />설명하는 코딩</strong><p>방문과 화상 수업 모두 아이의 속도에 맞춰 진행합니다.</p></aside></section>
